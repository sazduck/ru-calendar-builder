import type { Result, Transfer } from "./types";

export const parseTransfers = (str: string): Result<Transfer[], string> => {
  ...
  for (let m of matches) {
    ...
    const rawMonthFrom = m[2];
    const dayTo = m[3] ?? '';
    const isDayToLegal = Number(dayTo) <= 0 || Number(dayTo) > 31;
    if (isDayToLegal) {
      return { ok: false, error: `неверное число: ${m[1]}` };
    }
    const rawMonthTo = m[4];

    if (!rawMonthFrom || !rawMonthTo) {
      return { ok: false, error: 'ошибка структуры совпадения в регулярном выражении' };
    }
    const monthFrom = months[rawMonthFrom.toLowerCase()];
    const monthTo = months[rawMonthTo.toLowerCase()];
